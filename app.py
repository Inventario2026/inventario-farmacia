    ...
    with lock:
        if os.path.exists(ARCHIVO_KARDEX):
            try:
                ...
                return pd.DataFrame(columns=["ID", "Fecha y Hora", "Usuario", "Acción", "Producto", "Código", "Presentación", "Cantidad"])
        return pd.DataFrame(columns=["ID", "Fecha y Hora", "Usuario", "Acción", "Producto", "Código", "Presentación", "Cantidad"])
...
with pestana_registro:
    ...
    if df_productos is None:
        ...
        st.write("---")
        if st.button("💾 Registrar Movimiento", use_container_width=True, key="btn_manual"):
            ...
            nuevo_movimiento = {
                ...
                "Presentación": presentacion_final, "Cantidad": cantidad
            }
...
with pestana_foto:
    ...
    if foto_capturada is not None:
        ...
        st.warning("🤖 [Nota de Desarrollo]: La foto se guardó en el sistema. Para que la IA lea el texto exacto automáticamente, en el siguiente paso la conectaremos con un lector óptico (OCR). Por ahora, puedes usar el registro manual en la pestaña de al lado.")
        ...
        if df_kardex.empty:
            ...
            for idx, fila in df_kardex.iterrows():
                with st.container():
                    ...
                    with col_detalles:
                        st.markdown(f"📅 **{fila['Fecha y Hora']}** | 👤 *{fila['Usuario']}* | ⚡ {fila['Acción']} | 📦 **{fila['Producto']}** | 🔢 Cantidad: {fila['Cantidad']}")
                    with col_boton:
